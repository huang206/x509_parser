...
if __name__ == '__main__':
    ...
    try:
        count_str = "SELECT COUNT(connection_id) FROM ISI_certs"
        cursor.execute(count_str)
        row = cursor.fetchone()
        count  = row[0] if row else 0
    except Exception,e:
        print ("SELECT COUNT FROM ISI_certs ERROR:%s" %(`e`))
    ...
    try:
        while end < count:
            query_str = "SELECT pem_str FROM ISI_certs LIMIT %d,%d" %(start,end)
        ...
        query_str = "SELECT pem_str FROM ISI_certs LIMIT %d,%d" %(start,count)
        cursor.execute(query_str)
        row = cursor.fetchone()
        while row:
            if parseAndUpdateDB(str(row[0])):
        ...
        print ("SELECT FROM ISI_certs ERROR:%s" %(`e`))
